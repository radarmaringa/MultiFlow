...
const useStyles = makeStyles((theme) => ({
  ...
  queueTag: {
    background: theme.mode === "light" ? "#f1f3f5" : "#2a2a2a",
    color: theme.mode === "light" ? "#212529" : "#ffffff",
    marginRight: 4,
    padding: "4px 8px",
    fontWeight: "600",
    borderRadius: "6px",
    fontSize: "0.7em",
    whiteSpace: "nowrap",
    boxShadow: theme.mode === "light" ? "0 1px 2px rgba(0,0,0,0.05)" : "0 1px 2px rgba(0,0,0,0.2)",
  },
  ...
  contactLastMessage: {
    paddingRight: "0%",
    marginLeft: "8px",
    color: theme.mode === "light" ? "#495057" : "#adb5bd",
    fontSize: "0.875rem",
    [theme.breakpoints.down("sm")]: {
      marginLeft: "4px",
      fontSize: "0.8rem",
  ...
  waitingTimeGreen: {
    color: "#28a745", // Verde para até 5 minutos
  },
  waitingTimeOrange: {
    color: "#ff9800", // Laranja para 5-15 minutos
  },
  waitingTimeRed: {
    color: "#f44336", // Vermelho para mais de 30 minutos
  },
}));
...
const TicketListItemCustom = ({ setTabOpen, ticket }) => {
  ...
  const getWaitingTimeClass = (createdAt) => {
    if (!createdAt) return classes.waitingTime;
    ...
    if (minutes >= 30) {
      return `${classes.waitingTime} ${classes.waitingTimeRed}`;
    } else if (minutes >= 15) {
      return `${classes.waitingTime} ${classes.waitingTimeOrange}`;
    } else if (minutes >= 5) {
      return `${classes.waitingTime} ${classes.waitingTimeGreen}`;
    } else {
      return classes.waitingTime;
    }
  };
            ...
            <span className={classes.contactNameWrapper}>
              ...
              >
                ...
                {ticket.status === "pending" && ticket.createdAt && (
                  <span className={getWaitingTimeClass(ticket.createdAt)}>
                  ...
                  )}
                  {
                    <Badge
                      style={{
                        ...
                        background: ticket.queue?.color || "#7c7c7c", // Sobrescreve o gradiente da classe connectionTag
                      }}
                      className={classes.connectionTag}
                    >
                      {ticket.queueId
