...
const App = () => {
  ...
  // Novo estado para o modo do tema (classic, whatsapp-desktop, whatsapp-mobile)
  const [themeModeState, setThemeModeState] = useState(() => {
    return window.localStorage.getItem("themeMode") || "classic";
  });

  // Aplicar themeLayout do usuário quando disponível
  useEffect(() => {
    const savedThemeMode = window.localStorage.getItem("themeMode");
    if (!savedThemeMode || savedThemeMode === "classic") {
      // Se não há themeMode salvo ou é classic, aplica "classic"
  ...
  const colorMode = useMemo(
    () => ({
      ...
      appName,
      mode,
      themeMode: themeModeState, // Adiciona themeMode ao contexto
    }),
    [appLogoLight, appLogoDark, appLogoFavicon, appName, mode, themeModeState]
  );
