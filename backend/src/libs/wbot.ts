  ...
  return new Promise(async (resolve, reject) => {
    try {
      (async () => {
        ...
        // Busca versão do WhatsApp com cache para evitar rate limit do GitHub
        const versionWA = await getVersionByIndexFromUrl(2);
        console.log("[WBOT.ts] Versao sendo usada:", versionWA);

        const { state, saveCreds } = await useMultiFileAuthState(whatsapp);

        wsocket = makeWASocket({
          version: versionWA, // Versão dinâmica com cache (24h) para evitar rate limit
