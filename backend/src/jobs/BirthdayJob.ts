...
import { startUnattendedTicketJob } from "./UnattendedTicketJob"; // 🆕 NOVO IMPORT
...
/**
 * Inicializa todos os jobs relacionados a aniversários e notificações
 */
export const initializeBirthdayJobs = () => {
  ...
  const unattendedTicketJob = startUnattendedTicketJob(); // 🆕 NOVO JOB
  ...
  const shutdown = () => {
    ...
    cleanupJob.stop();
    unattendedTicketJob.stop(); // 🆕 NOVO JOB
  };
