...
export default async function ListTicketsServiceReport(
  ...
  select 
    ...
    coalesce((
      CASE 
        WHEN tt."startedAt" IS NOT NULL AND tt."queuedAt" IS NOT NULL THEN
          (date_part('day', age(tt."startedAt", tt."queuedAt"))) || ' d, ' || 
          (date_part('hour', age(tt."startedAt", tt."queuedAt"))) || ' hrs e ' ||
          (date_part('minutes', age(tt."startedAt", tt."queuedAt"))) || ' m'
        WHEN tt."startedAt" IS NULL AND tt."queuedAt" IS NOT NULL THEN
          (date_part('day', age(CURRENT_TIMESTAMP, tt."queuedAt"))) || ' d, ' || 
          (date_part('hour', age(CURRENT_TIMESTAMP, tt."queuedAt"))) || ' hrs e ' ||
          (date_part('minutes', age(CURRENT_TIMESTAMP, tt."queuedAt"))) || ' m'
        ELSE '0'
      END
    ), '0') "waitTime",
