        ...
        coalesce((
          CASE 
            ...
            WHEN tt."startedAt" IS NULL AND tt."queuedAt" IS NOT NULL THEN
              (EXTRACT(day FROM age(CURRENT_TIMESTAMP, tt."queuedAt")) * 24 * 60) +
              (EXTRACT(hour FROM age(CURRENT_TIMESTAMP, tt."queuedAt")) * 60) +
              (EXTRACT(minutes FROM age(CURRENT_TIMESTAMP, tt."queuedAt")))
            ELSE 0
          END
        ), 0) "waitTime",
        t.status,
